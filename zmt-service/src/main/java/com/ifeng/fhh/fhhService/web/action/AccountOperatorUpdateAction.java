...
import com.ifeng.fhh.fhhService.model.transform.account.AccountOperatorUpdateReq;
...
public class AccountOperatorUpdateAction implements RouteAction {

    private static final Logger LOG = LoggerFactory.getLogger(AccountOperatorUpdateAction.class);

    @Autowired
    private AccountService accountService;

    @Override
    @RequestMapping(path = "/operator/account/update/", method = RequestMethod.POST)
    public CompletableFuture<ActionResponse> route(ActionRequest request) {
        ...
        try {
            ...
            AccountOperatorUpdateReq accountSaveReq = JSON.parseObject(bodyJson, AccountOperatorUpdateReq.class);
            ...
            LOG.error("execute account operator update fail: {}", e);
