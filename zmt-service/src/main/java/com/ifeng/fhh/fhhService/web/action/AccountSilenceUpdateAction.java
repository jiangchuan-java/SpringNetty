...
import com.ifeng.fhh.fhhService.model.transform.account.AccountSilenceUpdateReq;
...
public class AccountSilenceUpdateAction implements RouteAction {

    private static final Logger LOG = LoggerFactory.getLogger(AccountSilenceUpdateAction.class);

    @Autowired
    private AccountService accountService;

    @Override
    @RequestMapping(path = "/silencer/account/update/", method = RequestMethod.POST)
    public CompletableFuture<ActionResponse> route(ActionRequest request) {
        ...
        try {
            ...
            AccountSilenceUpdateReq accountSaveReq = JSON.parseObject(bodyJson, AccountSilenceUpdateReq.class);
            ...
            LOG.error("execute account silence update fail: {}", e);
